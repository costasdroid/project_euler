...
public class Solution107 {
	...
	public static void main(String args[]) {
		...
		// select a big enough number to replace "-"
		int arbitraryBig = 10000;
		// make rows of data
		String[] data = s.replaceAll("-", "" + arbitraryBig).split("[\n]");
		// count inputs
		int inputs = data.length;
		// create matrix entries and sum all values
		int[][] matrix = new int[inputs][inputs];
		int totalSum = 0;
		...
		for (int i = 0; i < inputs; i++) {
			...
			for (String s : data[i].split("[,]")) {
				...
				if (matrix[i][j] < arbitraryBig)
					totalSum = totalSum + matrix[i][j];
				j++;
			}
		}

		// create nodes added each repetition
		ArrayList<Integer> nodes = new ArrayList<Integer>();

		// choose any initial node
		nodes.add(0);

		int sum = 0;

		// for every node added so far, find the next min edge to a non existing
		// node
		while (nodes.size() < inputs) {
			int min = arbitraryBig;
			int newNode = 0;
			for (int node : nodes) {
				for (int j = 0; j < inputs; j++) {
					if (!nodes.contains(j) && matrix[node][j] > 0 && matrix[node][j] < min) {
						min = matrix[node][j];
						newNode = j;
					}
				}
			}
			// System.out.println("New node " + newNode + " with weight " +
			// min);
			nodes.add(newNode);
			sum += min;
		}

		int sol = totalSum / 2 - sum;
		System.out.println("Took: " + (System.currentTimeMillis() - t) + "ms");
		System.out.println("Answer: " + sol);

	}

	static String s = "-,-,-,427,668,495,377,678,-,177,-,-,870,-,869,624,300,609,131,-,251,-,-,-,856,221,514,-,591,762,182,56,-,884,412,273,636,-,-,774\n"
